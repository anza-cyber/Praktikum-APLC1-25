...
string rupiahFormat(double angka) {
    string s = to_string((long long)angka);
    int n = s.size();
    string hasil = "";
    for (int i = 0; i < n; i++) {
...
int tampilTabel(int stockOnly = 0) {
    cout << "\n+----+--------------------+----------------+---------+-------------+\n";
...
int doRegister() {
    ...
    for (int i = 0; i < jumlahPengguna; i++) {
        if (dataPengguna[i].nama == nama) {
            cout << "Yah, nama itu udah dipake orang lain, pake nama lain...\n";
            return 0;
        }
    }
    ...
    cout << "Daftar sebagai (admin / pelanggan): ";
    getline(cin, role);

    if (role != "admin" && role != "pelanggan") {
        cout << "Pilihannya cuma admin atau pelanggan ya!\n";
        return 0;
    }
    ...
    cout << "\nYes! Akun '" << nama << "' berhasil dibuat sebagai " << role << "!\n";
    return 1;
}
...
int tambahBunga() {
    ...
    bunga.harga.satuan   = stod(hargaStr);
...
int pilihNomor(string aksi) {
    ...
    if (no == 0) { cout << "Belum ada bunga nih...\n"; return -1; }
...
int editBunga() {
    ...
    if (hargaBaru.size() > 0) {
        if (validAngka(hargaBaru) == 1) bunga.harga.satuan = stod(hargaBaru);
...
int hapusBunga() {
    ...
    cout << "Penghapusan dibatalin.\n";
    return 0;
}

int menuAdmin(string namaUser) {
    string pilihan;
    while (true) {
        cout << "\n==================================================\n";
...
int beliBunga() {
    ...
    if (adaStok == 0) { cout << "Maaf ya, semua bunga lagi habis...\n"; return 0; }
...
int menuPelanggan(string namaUser) {
    string pilihan;
    while (true) {
        cout << "\n==================================================\n";
        ...
        else if (pilihan == "0") { cout << "Makasih udah mampir! Sampai jumpa lagi :)\n"; return 0; }
...
>>>>>>> 8a8a49898d561f7e3e2d857ddb3fe5fd1004c828
    }
