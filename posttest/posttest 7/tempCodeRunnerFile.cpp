...
void tambahBunga() {
    ...
    try {
        if (totalBunga >= MAKS_BUNGA)
            throw runtime_error("Data bunga udah penuh nih!");
        ...
        if (validAngka(hargaStr) == 0)
            throw invalid_argument("Harga sama stok harus angka ya!");

        cout << "Stok        : "; getline(cin, stokStr);

        if (validAngka(stokStr) == 0)
            throw invalid_argument("Harga sama stok harus angka ya!");
...
void cariNama() {
    ...
    try {
        ...
        if (jml == 0)
            throw runtime_error("Belum ada data bunga.");
        ...
        if (keyword.empty())
            throw invalid_argument("Keywordnya gak boleh kosong dong!");
...
void cariHarga() {
    ...
    try {
        ...
        if (validAngka(inputHarga) == 0)
            throw invalid_argument("Harganya harus angka ya!");
        ...
        if (posisi == -1)
            throw runtime_error("Bunga dengan harga " + rupiahFormat(target) + " gak ada.");
...
void beliBunga() {
    ...
    try {
        ...
        if (adaStok == 0)
            throw runtime_error("Maaf ya, semua bunga lagi habis nih...");
        ...
        if (validAngka(pilihanBeli) == 0 || bacaAngka(pilihanBeli) < 1 || bacaAngka(pilihanBeli) > no)
            throw invalid_argument("Nomornya gak valid!");
        ...
        if (validAngka(pilihanJumlah) == 0 || bacaAngka(pilihanJumlah) <= 0)
            throw invalid_argument("Jumlahnya gak valid!");

        int jumlah = bacaAngka(pilihanJumlah);
        if (jumlah > bunga.stok.jumlah)
            throw runtime_error("Stok gak cukup! Cuma ada " + angkaKeStr(bunga.stok.jumlah) + " " + bunga.stok.satuan + ".");
    ...
    try {
        ...
        for (int i = 0; i < jumlahPengguna; i++) {
            if (dataPengguna[i].nama == nama)
                throw runtime_error("Yah, nama itu udah dipake orang lain, pake nama lain ya!...");
        }
        ...
        if (password.empty())
            throw invalid_argument("Password gak boleh kosong!");
