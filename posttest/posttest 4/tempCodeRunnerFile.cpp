...
// Fungsi login menggunakan parameter Pengguna* data (pointer) dan int jml (pass by value)
// Address-of (&) digunakan saat pemanggilan: doLogin(dataPengguna, jumlahPengguna)
int doLogin(Pengguna* data, int jml) {
    int coba = 0;
    while (coba < 3) {
...
int main() {
    ...
    while (berjalan) {
        ...
        if      (pilihan == "1") doRegister();
            ...
            // Penggunaan address-of (&) saat passing array ke fungsi doLogin
